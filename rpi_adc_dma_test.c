...
int main(int argc, char *argv[])
{
    ...
    //spi_freq = MIN(sample_rate * RX_SAMPLE_SIZE * 8, MAX_SPI_FREQ);
    ...
    if (sample_count == 0)
    {
        ...
        printf("ADC value in count %u = %4.3fV\n", val, val*3.3/1024.0);
    }
    else
    {
...
void dma_wait(int chan)
{
    ...
    if (n == 0){
	    ...
	    //printf("Value of n is %d and channel is %d\n",n,chan);
...
//#define GPIO_BASE 0x3F200000  // For RPi 3/4; use 0x20200000 for RPi 1
#define GPIO_INPUT 0x0        // Input mode

volatile unsigned int* gpio = (unsigned int*)GPIO_BASE;

void reset_spi() {
    // Assuming base address is mapped correctly
    gpio[SPI0_MOSI_PIN / 10] &= ~(7 << ((SPI0_MOSI_PIN % 10) * 3)); // Clear bits
    gpio[SPI0_MOSI_PIN / 10] |= (GPIO_INPUT << ((SPI0_MOSI_PIN % 10) * 3)); // Set input mode
    // Repeat for SPI_MISO_PIN, SPI_CLK_PIN, etc.
}
